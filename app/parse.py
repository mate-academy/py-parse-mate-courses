...
def get_course_info_soup(course_soup: BeautifulSoup):
...
def parse_course_name(course_soup: BeautifulSoup) -> Course:
    return Course(
        ...
        short_description=get_course_info_soup(course_soup).find(
            class_="typography_landingTextMain__Rc8BD SalarySection_aboutProfession__1VFHK"
        ).text,
        duration=get_course_info_soup(course_soup).find_all(
            class_="ComparisonTable_cell__RNsyU ComparisonTable_highlightCell__IpyWZ typography_newLandingH6__Av9tT"
        )[12].text
    )
...
def get_all_courses():
    ...
    courses = base_page_soup.select(".HeaderCoursesDropdown_coursesListItem__36tl_")
